"use strict";
(self["webpackChunkjcduplan"] = self["webpackChunkjcduplan"] || []).push([[792],{
...
var tiny_slider = __webpack_require__(473);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  requestAnimationFrame(raf);
  const preloaderTl = gsapWithCSS.timeline({
    defaults: {
      ease: "ease-out-quart",
      duration: 1.5,
      delay: 0.5
  ...
  preloaderTl.to("#js-loader-image", {
    y: 0,
    autoAlpha: 1,
    duration: 0.8
  ...
  const observerFade = new IntersectionObserver(observerCallbackFade, observerOptionsFade);
  elementsFade.forEach(el => observerFade.observe(el));
  const elementsTranslateY = new Set([...document.querySelectorAll("[fade-translate-y]")]);
  const observerOptionsTranslateY = {
    root: null,
    rootMargin: "0px",
    threshold: 0.5
  };
  const observerTranslateY = new IntersectionObserver(function (entries, observer) {
    var delay = 0;
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        setTimeout(function () {
  ...
  const elementsContrast = new Set([...document.querySelectorAll("[contrast]")]);
  ...
  const observerContrast = new IntersectionObserver(observerCallbackContrast, observerOptionsContrast);
  elementsContrast.forEach(el => observerContrast.observe(el));
  var sliderAgenda = (0,tiny_slider/* tns */.U)({
    container: ".js-slider",
    items: 1,
    controls: true,
    navPosition: "bottom",
